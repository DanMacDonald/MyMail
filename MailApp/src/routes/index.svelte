<script lang="ts">
	...
	let emailInboxItems = $keyStore.emailInboxItems || [];
	let weaveMailInboxItems = $keyStore.weaveMailInboxItems || [];
	...
	keyStore.subscribe((store) => {
        ...
        if (store.bridgeUrl != undefined) {
			...
			emailInboxItems = [];
			inboxItems = weaveMailInboxItems;
		}
    });
	...
	async function getWeamailItems() {
		console.log(weaveMailInboxItems);
		if (weaveMailInboxItems.length > 0) {
			promise = new Promise((res) => { res(weaveMailInboxItems); });
		}

		var address = await arweave.wallets.jwkToAddress(wallet);
		let json = await getWeavemailTransactions(address);
		weaveMailInboxItems = await Promise.all(
		...
		inboxItems = emailInboxItems;

		if (weaveMailInboxItems.length > 0)
			inboxItems.push(...weaveMailInboxItems);

		$keyStore.weaveMailInboxItems = weaveMailInboxItems;
	...
	async function getInbox() {
		if ($keyStore.bridgeUrl == undefined) return [];
